...
def display_memory_usage():
    ...
    if memory_percentage > MEMORY_THRESHOLD:
        ...
        alert = st.sidebar.warning("🧹 Đã tự động dọn dẹp bộ nhớ!")
        time.sleep(3)
        alert.empty()
...
def add_memory_cleanup_button():
    if st.sidebar.button("🧹 Dọn dẹp bộ nhớ"):
        ...
        if memory_saved > 0:
            alert = st.sidebar.success(f"✅ Đã dọn dẹp bộ nhớ! Giải phóng được {memory_saved:.1f} MB")
            time.sleep(3)
            alert.empty()
        else:
            alert = st.sidebar.info("ℹ️ Không có bộ nhớ nào được giải phóng. Hệ thống đã tối ưu.")
            time.sleep(3)
            alert.empty()